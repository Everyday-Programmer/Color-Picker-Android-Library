...
public class HueSatView extends SquareView implements ColorObserver {
	...
	private void update() {
		observableColor.updateHueSat(hueForPos(pointer.x, pointer.y, w), satForPos(pointer.x, pointer.y, w), this);
		optimisePointerColor();
		invalidate();
	}
	...
	private void optimisePointerColor() {
		pointerPaint.setColor(observableColor.getLightnessWithValue(1) > 0.5 ? 0xff000000 : 0xffffffff);
	}
	...
	private static Bitmap makeBitmap(int radiusPx) {
		...
		for (int y = 0; y < radiusPx; ++y) {
			for (int x = 0; x < radiusPx; ++x) {
				...
				int alpha = (int)(Math.max(0, Math.min(1, (1 - sat) * radiusPx)) * 255);
				if (alpha > 0) {
